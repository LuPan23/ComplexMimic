...
from complexmimic.utils.motion_lib_base import MotionLibBase, compute_motion_dof_vels, FixHeightMode
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, shape_params, mesh_parsers, config, queue, pid):
        ...
        for f in pbar:
            ...
            if (not flags.im_eval) and (not flags.test):
                ...
                trans = torch.matmul(
                    trans,
                    torch.from_numpy(random_heading_rot.as_matrix().T).to(dtype=trans.dtype, device=trans.device)
                )
